...
.hdr{background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);color:#fff;padding:16px 24px 14px;text-align:center}
.hdr-inner{display:flex;align-items:center;justify-content:space-between;gap:16px}
.hdr-text{flex:1;text-align:center}
.hdr-logo-wrap{display:flex;flex-direction:column;align-items:center;gap:5px;flex-shrink:0}
.hdr-logo{width:80px;height:80px;object-fit:contain;border-radius:50%;background:#fff;padding:4px;border:3px solid #FFD700;box-shadow:0 0 15px rgba(255,215,0,.6),0 4px 15px rgba(0,0,0,.3);transition:transform .3s,box-shadow .3s;flex-shrink:0}
.hdr-logo:hover{transform:scale(1.08);box-shadow:0 0 25px rgba(255,215,0,.9),0 6px 20px rgba(0,0,0,.4)}
.hdr-regd{font-size:10px;color:#FFD700;font-weight:700;letter-spacing:.3px;text-align:center;white-space:nowrap;text-shadow:0 1px 3px rgba(0,0,0,.4)}
...
.sc{flex:1;border-right:1px solid rgba(255,255,255,.15);padding:16px 10px;text-align:center;transition:filter .2s}
.sc:hover{filter:brightness(1.08)}
.sc:last-child{border-right:none}
.sc .v{font-size:24px;font-weight:700;line-height:1.2}
.sc .l{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.7px;margin-top:4px;opacity:.85}
/* KPI card colours */
.sc-props {background:linear-gradient(135deg,#1a3c5e,#0f3460);color:#fff}
.sc-billed{background:linear-gradient(135deg,#0369a1,#0284c7);color:#fff}
.sc-paid  {background:linear-gradient(135deg,#15803d,#16a34a);color:#fff}
.sc-due   {background:linear-gradient(135deg,#b91c1c,#dc2626);color:#fff}
/* ── Search toolbar ──────────────────────────────────────── */
...
.sumtbl{table-layout:fixed}
.sumtbl th{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:6px 8px}
.sumtbl td{padding:3px 8px;line-height:1.3}
.sum-lane{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:0;color:#1a1a2e;font-weight:500}
.sum-plots{font-size:12px;color:inherit;font-weight:400;margin-left:2px}
/* highlight match */
...
table{width:100%;table-layout:fixed;border-collapse:collapse;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.07)}
th{background:#1a3c5e;color:#fff;padding:6px 8px;text-align:left;font-size:12px;font-weight:600;letter-spacing:.3px;white-space:nowrap}
td{padding:2px 6px;border-bottom:1px solid #f1f5f9;vertical-align:middle;font-size:12px;font-family:'Segoe UI',Arial,sans-serif;border-right:1px solid #eef2f7;line-height:1.3}
td:last-child{border-right:none}
th:last-child{border-right:none}
tr:last-child td{border-bottom:none}
tbody tr:hover td{background:#f8fafc}
.lh td{background:#e8edf8;color:#1a3a6b;font-size:12px;font-weight:700;padding:5px 8px;border-top:2px solid #1a3c5e;border-bottom:1px solid #b0bdd8}
.lmeta{font-size:10px;font-weight:400;color:#555;margin-left:8px}
.gt td{background:#1a3c5e;color:#fff;font-size:12px;font-weight:700;padding:5px 8px}
...
.c-sno{color:#94a3b8;text-align:center}
.c-pid{font-weight:700;color:#1a3a6b}
.c-plot{color:#334155;white-space:nowrap}
.c-name{color:#1a1a2e;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:0;cursor:default}
...
.c-act-wrap .btn-mail,.c-act-wrap .btn-wa,.c-act-wrap .btn-sms,.c-act-wrap .act-slot{
  ...
  width:20px;height:20px;border-radius:4px;font-size:10px;font-weight:800;
...
JS_LOAD = r"""
// ── Cache constants ────────────────────────────────────────
const CACHE_VERSION  = '202607032315';          // bumped on every rebuild
const CACHE_KEY      = 'scrwaReportData_'  + CACHE_VERSION;
const CACHE_TIME_KEY = 'scrwaReportTime_'  + CACHE_VERSION;
const CACHE_DURATION = 4 * 60 * 60 * 1000; // 4 hours — hard limit
let   _autoRefreshTimer = null;

// Auto-purge all OLD version keys on load
(function pruneOldCache() {
  try {
    Object.keys(localStorage).forEach(k => {
      if ((k.startsWith('scrwaReportData_') || k.startsWith('scrwaReportTime_'))
...
function showError(msg) {
  ...
  div.innerHTML = '<div style="font-size:32px">❌</div><h3 style="color:#dc2626;margin:8px 0">Failed to load data</h3><p style="color:#64748b;font-size:13px">' + msg + '</p><button onclick="this.parentNode.remove();loadData(true)" style="margin-top:12px;padding:8px 20px;background:#1a3c5e;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px">🔄 Retry</button>';
  document.body.appendChild(div);
}

function setLivePill(mode, ts) {
  const pill = document.getElementById('livePill');
  if (!pill) return;
  const t = ts ? new Date(ts).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:true}) : '';
  if (mode === 'loading') {
    pill.className = 'live-pill loading';
    pill.textContent = '⏳ Loading\u2026';
  } else if (mode === 'live') {
    pill.className = 'live-pill live';
    pill.textContent = '🟢 Live' + (t ? '  ·  Updated ' + t : '');
  } else if (mode === 'cached') {
    const ageMs  = ts ? Date.now() - ts : 0;
    const ageHrs = ageMs / (60*60*1000);
    const stale  = ageHrs >= 3.5;
    pill.className = 'live-pill cached' + (stale ? ' stale' : '');
    pill.innerHTML = (stale ? '🔴' : '🟡') + ' Cached' + (t ? ' · ' + t : '')
      + (stale ? ' <span style="color:#dc2626;font-size:10px;font-weight:700">(Stale — refreshing...)</span>' : '')
      ...
      + '🔄 Refresh</button>';
    if (stale) setTimeout(() => loadData(true), 1500); // auto-trigger refresh if stale
  } else if (mode === 'error') {
...
function startAutoRefresh() {
  ...
  _autoRefreshTimer = setInterval(() => {
    ...
    if (t && (Date.now() - parseInt(t)) >= CACHE_DURATION) {
      console.log('SCRWA: Cache ≥ 4hrs — auto-refreshing...');
      loadData(true);
    }
  }, 60 * 1000); // check every 1 min — triggers at exactly 4hr mark
}
...
function loadData(forceRefresh) {
  ...
  // ── Use localStorage cache if < 4 hours old ─────────────────────
  if (!forceRefresh) {
    try {
      const cached  = localStorage.getItem(CACHE_KEY);
      const cacheTs = localStorage.getItem(CACHE_TIME_KEY);
      if (cached && cacheTs && (Date.now() - parseInt(cacheTs)) < CACHE_DURATION) {
        if (overlay) overlay.style.display = 'none';
        renderReport(JSON.parse(cached));
        setLivePill('cached', parseInt(cacheTs));
        startAutoRefresh();
        return;
      }
  ...
  // ── Fetch live from Google Apps Script ─────────────────────
  const API = 'https://script.google.com/macros/s/AKfycbxwEAILeFAdVPJGE1RnpSFb7TP48gp-JHHF6-gXx-vhD59P_gTxV9fxIuFCGS-JDfY/exec';
  fetch(API, { signal: AbortSignal.timeout(30000) })
    ...
    .then(data => {
      ...
      renderReport(data);
      setLivePill('live', Date.now());
      startAutoRefresh();
    })
    .catch(err => {
      console.warn('SCRWA live fetch failed:', err);
      // Fallback 1: stale localStorage
      try {
        const cached = localStorage.getItem(CACHE_KEY);
        if (cached) {
          if (overlay) overlay.style.display = 'none';
          ...
          setLivePill('cached', ts ? parseInt(ts) : null);
          startAutoRefresh();
          return;
        }
      } catch(e) {}
      // Fallback 2: bundled live_data.json
      fetch('live_data.json')
        .then(r => r.json())
        .then(data => {
          if (overlay) overlay.style.display = 'none';
          renderReport(data);
          setLivePill('cached', null);
        })
        .catch(() => showError('Could not load data from API or local cache.'));
    });
}
...
function doLogin() {
  ...
  if (ADMINS[email]) {
    ...
    loadData(true); // always fetch fresh on admin login
  } else {
...
function fmtName(raw) {
  ...
  return '<span title="' + raw.replace(/"/g, '&quot;') + '">' + parts[0].trim() + ' <span style="color:#94a3b8;font-size:11px">&amp; \u2026</span></span>';
}
...
function renderReport(raw) {
  ...
  summaryRows.forEach(r => {
    ...
    const metaStr  = r.lane.pids.length + ' props  ·  Billed: ' + ifmt(r.bTot)
                   ...
                   + (pillStr ? '   ' + pillStr : '');
    detHTML += '<tr class="lh" data-lane="' + r.lane.name + '">'
             + '<td colspan="11">\uD83C\uDFE0 ' + r.lane.name + ' — ' + r.lane.plots
             + ' <span class="lmeta">' + metaStr + '</span></td></tr>';
...
function rbadge(prop, pid, due) {
  ...
  const laneName = prop.laneName || laneNo || '';
  ...
  if (due > 0 && email) {
    ...
    const det =
      ...
      + ' (' + months + ' month' + (months > 1 ? 's' : '') + ' pending)\n'
      + '📅 As On        : ' + asOn
      + (proxy ? '\n👤 Proxy/Rep    : ' + proxy : '');
    const subj = encodeURIComponent(
      '🔔 Friendly Reminder – Outstanding ₹' + due.toLocaleString('en-IN')
      + ' (' + months + ' month' + (months > 1 ? 's' : '') + ' pending) – Plot ' + plotNo
    );
    const body = encodeURIComponent(
        'Dear ' + name + ',\n\n'
      + 'Greetings from the SCRWA Management Committee, Vampuguda, Hyderabad!\n\n'
      + 'Our society records reflect that you have the following outstanding contribution:\n\n'
      + divider + '\n'
      + det + '\n'
      + divider + '\n\n'
      + 'We kindly request you to clear the above dues at your earliest convenience.\n'
      + 'Your timely support helps maintain and improve our community for everyone.\n\n'
      + '💳 Scan & Pay (UPI/GPay/PhonePe): https://scwavampuguda-tech.github.io/society-dashboard/Society_Files/HTML_Portals/payment_scanner.jpg\n\n'
      + '🔗 View Statement: https://scwavampuguda-tech.github.io/society-dashboard/Society_Portal.html\n'
      + '📧 Queries: scwa.vampuguda@gmail.com\n\n'
      + 'Thank you for your cooperation.\n\n'
      + 'Warm regards,\n'
      + 'Management Committee\n'
      + 'Senior Citizens Residential Welfare Association (SCRWA)\n'
      + 'Regd. No: 2240/2006'
    );
    const to = email + (proxy && prop.proxyEmail ? ',' + prop.proxyEmail : '');
    gBtn = '<a class=\"btn-mail\" href=\"https://mail.google.com/mail/u/0/?view=cm&fs=1&to='
      + encodeURIComponent(to) + '&su=' + subj + '&body=' + body
      + '\" target=\"_blank\" title=\"Gmail \u2014 ' + email + '\">G</a>';
  }
  if (due > 0 && phone) {
    const wp = phone.replace(/[^0-9]/g, '');
    const wm = encodeURIComponent(
        'Friendly Reminder - SCRWA\n'
      + 'Dear ' + name + ',\n\n'
      + 'Greetings from SCRWA Management Committee, Vampuguda, Hyderabad!\n\n'
      + 'Our society records reflect that you have the following outstanding contribution:\n\n'
      + 'Property ID  : ' + pid      + '\n'
      + 'Plot No      : ' + plotNo   + '\n'
      + 'Lane         : ' + laneName + '\n'
      + 'Outstanding  : Rs.' + due.toLocaleString('en-IN')
      + ' (' + months + ' month' + (months > 1 ? 's' : '') + ' pending)\n'
      + 'As On        : ' + asOn + '\n'
      + (proxy ? 'Proxy/Rep    : ' + proxy + '\n' : '')
      + '\nWe kindly request you to clear the above dues at your earliest convenience.\n'
      + 'Your timely support helps maintain and improve our community for everyone.\n\n'
      + 'Scan & Pay (UPI/GPay/PhonePe): https://scwavampuguda-tech.github.io/society-dashboard/Society_Files/HTML_Portals/payment_scanner.jpg\n\n'
      + 'View Statement: https://scwavampuguda-tech.github.io/society-dashboard/Society_Portal.html\n'
    ...
    const waNum = wp.length === 10 ? '91' + wp : wp;
    const waUrl = 'https://wa.me/' + waNum + '?text=' + wm;
    wBtn = '<a class=\"btn-wa\" href=\"' + waUrl + '\" target=\"_blank\" title=\"WhatsApp \u2014 ' + phone + '\">W</a>';
    sBtn = '<a class=\"btn-sms\" href=\"sms:' + phone.replace(/[^0-9+]/g, '') + '\" title=\"SMS \u2014 ' + phone + '\">S</a>';
  }
...
<div class="hdr">
  <div class="hdr-inner">
    <div class="hdr-logo-wrap">
      <img src="icons/society_logo.png" class="hdr-logo" alt="SCRWA Logo" onerror="this.style.display='none'">
      <span class="hdr-regd">Regd No. 2240/2006</span>
    </div>
    <div class="hdr-text">
...
<!-- KPI Stats -->
<div class="stats">
  <div class="sc sc-props" ><div class="v" id="st-props"  >--</div><div class="l">&#127968; Total Properties</div></div>
  <div class="sc sc-billed"><div class="v" id="st-billed" >--</div><div class="l">&#128179; Total Billed</div></div>
  <div class="sc sc-paid"  ><div class="v" id="st-paid"   >--</div><div class="l">&#9989; Total Paid</div></div>
  <div class="sc sc-due"   ><div class="v" id="st-due"    >--</div><div class="l">&#128308; Total Outstanding</div></div>
</div>
...
<div class="sec">
  ...
  <table class="sumtbl">
    <thead><tr>
      <th style="width:34%;text-align:left">🏠 Lane</th>
      <th style="width:5%;text-align:center">Props</th>
      <th style="width:10%;text-align:right">💳 Billed</th>
      <th style="width:10%;text-align:right">✅ Paid</th>
      <th style="width:10%;text-align:right">💰 Due</th>
      <th style="width:5%;text-align:right">🔁 Adv</th>
      <th style="width:6%;text-align:center">✅ Paid</th>
      <th style="width:6%;text-align:center">🔶 Part</th>
      <th style="width:8%;text-align:center">❌ Unpaid</th>
      <th style="width:6%;text-align:center">— N/A</th>
    </tr></thead>
...
<div class="sec">
  ...
  <table>
    <thead><tr>
      <th style="width:3%">#</th>
      <th style="width:5%">🏷️ Prop ID</th>
      <th style="width:14%;white-space:nowrap">📍 Plot No</th>
      <th style="width:21%">👤 Owner Name</th>
      <th style="width:9%">📋 Status</th>
      <th style="width:9%">💳 Payment</th>
      <th style="width:9%;text-align:right">💳 Billed</th>
      <th style="width:8%;text-align:right">✅ Paid</th>
      <th style="width:8%;text-align:right">🔴 Due</th>
      <th style="width:6%;text-align:right">🔄 Advance</th>
...
<button class="fab refresh-btn" id="refreshBtn" onclick="loadData(true)">&#128260; Refresh Data</button>
...
# Auto post-process: fix literal newlines inside JS strings in rbadge block
import re as _re
def _fix_rbadge_js(h):
    s = h.find('function rbadge(prop')
    e = h.find('\n}\n', s) + 3
    blk = h[s:e]
    fixed = _re.sub(r"'([^']*)'", lambda m: "'" + m.group(1).replace('\n', r'\n') + "'", blk)
    return h[:s] + fixed + h[e:]
HTML = _fix_rbadge_js(HTML)

# Post-process: replace raw emoji chars in rbadge JS with \uXXXX surrogate pairs
# so encodeURIComponent in wa.me URLs works correctly on all browsers
def _emoji_to_js(html):
    def _esc(c):
        cp = ord(c)
    ...
    EMOJIS = '📢👋📋🏠📍🛣💰📅👤🙏💪🔗📧🤝🔔'
    s = html.find('function rbadge(prop')
    e = html.find('\n}\n', s) + 3
    blk = html[s:e]
    for em in EMOJIS:
        blk = blk.replace(em, _esc(em))
    return html[:s] + blk + html[e:]
HTML = _emoji_to_js(HTML)
