...
function rbadge(prop, pid, due) {
  ...
  if (due > 0 && email) {
    const subj = encodeURIComponent('Maintenance Due Reminder — ' + name + ' | Property ' + pid + (months > 1 ? ' (' + months + ' months)' : ' (1 month)'));
    const det  = 'Property ID   :  ' + pid + '\nPlot No       :  ' + plotNo + '\nLane          :  ' + laneNo + '\nContribution  :  \u20b9' + due.toLocaleString('en-IN') + ' (' + (months > 1 ? months + ' months' : '1 month') + ' pending)\nAs On         :  ' + asOn + (proxy ? '\nProxy / Rep   :  ' + proxy + ' (Proxy)' : '');
    const bod  = encodeURIComponent('Dear ' + name + ',\n\nHope this message finds you well.\n\nThis is a gentle reminder from the Management Committee of\nSenior Citizen Residential Welfare Association (SCRWA), Vampuguda, Hyderabad.\n\n' + det + '\n\nWe kindly request you to clear this balance at your earliest convenience.\n\nView your full statement on the Society Portal:\nhttps://scwavampuguda-tech.github.io/society-dashboard/Society_Portal.html\n\nmailto:scwa.vampuguda@gmail.com\n\nWarm regards,\nManagement Committee\nSenior Citizen Residential Welfare Association (SCRWA)\nRegd. No: 2240/2006');
    const to   = email + (proxy && prop.proxyEmail ? ',' + prop.proxyEmail : '');
    gBtn = '<a class="btn-mail" href="https://mail.google.com/mail/u/0/?view=cm&fs=1&to=' + encodeURIComponent(to) + '&su=' + subj + '&body=' + bod + '" target="_blank" title="Gmail — ' + email + '">G</a>';
  }
  if (due > 0 && phone) {
    const wp  = phone.replace(/[^0-9]/g, '');
    const wm  = encodeURIComponent('Dear ' + name + ', this is a gentle reminder from SCRWA for maintenance due of \u20b9' + due.toLocaleString('en-IN') + '. Please clear at your earliest convenience. — SCRWA Management');
    wBtn = '<a class="btn-wa"  href="https://wa.me/' + wp + '?text=' + wm + '" target="_blank" title="WhatsApp — ' + phone + '">W</a>';
